...
async function createConfigFile(replicaSettings, log) {
  ...
  let hostIPs = await ipUtil.getIPs({
    [process.env.THEMIS_REPLICA_HOST_PREFIX]:
  ...
  await fs.writeFile(
    path.join(
      process.env.THEMIS_DIR,
      process.env.THEMIS_CONFIG_FILE_PATH
    ),
    configString
  );
  await fs.writeFile(
    path.join(
      process.env.THEMIS_DIR,
      process.env.THEMIS_PBFT_CONFIG_FILE_PATH
    ),
    pbftString
  );
...
async function passArgs(hosts, clientSettings, log) {
  ...
  for (let i = 0; i < hosts.length; i++) {
    ...
    hosts[i].procs.push({
      path: path.join(
        process.env.THEMIS_DIR,
        process.env.THEMIS_REPLICA_BIN
      ),
      env: 'RUST_BACKTRACE=1',
...
async function getStats(experimentId, log) {
  let clientFile = await fs.readFile(
    path.join(
      path.join(
        process.env.THEMIS_EXPERIMENTS_OUTPUT_DIR,
        experimentId
      ),
      path.join(
